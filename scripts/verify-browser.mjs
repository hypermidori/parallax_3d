...
try{
  ...
  check('all approach and pooled city meshes have UVs and image textures',(await read()).materials.length===126&&(await read()).materials.every(m=>m.textured&&m.uv));
  ...
  let e=(await read()).enemies.slice().sort((a,b)=>Math.abs(a.screen.x)-Math.abs(b.screen.x))[0].screen;await page.mouse.move((e.x+1)*720,(1-e.y)*450);await page.mouse.down();await page.waitForFunction(()=>__nightVector.inspect().locks>1,null,{timeout:3000});
  ...
  await mobile.goto(origin+'?debug=1');await mobile.waitForFunction(()=>window.__nightVector);await mobile.locator('#start').tap();check('portrait touch viewport can start play',await mobile.evaluate(()=>__nightVector.inspect().state==='playing'));await mobile.setViewportSize({width:844,height:390});await mobile.waitForFunction(()=>__nightVector.inspect().enemies.length>0);
  ...
  await mobile.setViewportSize({width:390,height:844});await mobile.waitForTimeout(100);check('rotating portrait keeps play active',await mobile.evaluate(()=>__nightVector.inspect().state==='playing'));await mobile.setViewportSize({width:844,height:390});await mobile.waitForTimeout(100);check('rotating landscape keeps play active',await mobile.evaluate(()=>__nightVector.inspect().state==='playing'));
